...
def state_action_build(file_path, output_path):
    ...
    for item in tqdm(data[:]):
        problem = item['prompt']
        canonical_steps = '\n'.join(item['canonical_steps'])
        topic = item['topic']
        sa_pair.append({"problem": problem, "canonical_steps": canonical_steps, "topic": topic})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="../../output/knowledge_base/kb_1_v2")
    parser.add_argument("--emb_model_name", type=str, default="bge-large-en-v1.5")
    parser.add_argument("--content_key", type=str, default="problem")
