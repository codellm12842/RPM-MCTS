...
def get_trace(prog, funcname):
    ...
    import subprocess
    try:
        res=subprocess.run(["python3", "-m", "trace", "-t", fname], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=10)
...
def collect_runtime_value_simple(value_prof_prog):
    ...
    import subprocess
    try:
        res=subprocess.run(["python3", fname], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=10)
    except subprocess.TimeoutExpired:
        return "*timeout*"
    finally:
        os.remove(fname)
