class treeNode(object):
    ...
    def update_y_from_parent(self):
        if self.parent is None:
            # if it is root node, the y is the pcd
            self.y = self.pcd
        else:
            # if it is not root node, the y is the parent's y + the pcd
    ...
    def getBestV(self):  # 获取子树最大价值节点
        if not self.isFullyExpanded:
            # if it is not fully expanded, the best value is the current node's value
            return self, self.V
        max_V = self.V
        max_node = self
