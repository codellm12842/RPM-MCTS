...
def get_next_steps_roll(y: str, step_n: int, mcts_task):
    next_steps = []
    for _ in range(mcts_task.roll_branch):
        proposal = ''
        cnt = 3
...
def get_next_steps_expand(node: treeNode, mcts_task):
    ...
    history = []
    for _ in range(mcts_task.branch):
        proposal = ''
        cnt = 3
        while not proposal and cnt:
            if mcts_task.use_reflection == 'common':
                proposal = mcts_task.get_next_step_use_reflection(node.y, node.depth + 1, reflection, history)
            else:
                proposal = mcts_task.get_next_step(node.y, node.depth + 1, history)
            cnt -= 1
...
def randomPolicy(node: treeNode, mcts_task):
    ...
    if reflection == '<end>':
        print('此步问题已解决，不需要模拟。\n')
        return node.V
    for _ in range(mcts_task.roll_forward_steps):
...
def selectNode(node, mcts_task):
    while node.isFullyExpanded:
        ...
        print('-'*40 + 'In selectNode, node.isFullyExpanded is True' + '-'*40)
