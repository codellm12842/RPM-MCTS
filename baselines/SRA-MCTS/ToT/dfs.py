...
def DFS_sub(tot_task, node):
    ...
    for i in range(tot_task.branch):
        ...
        full_steps = tot_task.get_full_step(child.y, child.depth+1)
        value = tot_task.get_step_value(full_steps)
        child.full_steps = full_steps
    ...
    if ranked_candidates[0].V >= tot_task.end_gate:
        ...
        return ranked_candidates[0].full_steps, node, ranked_candidates[0]
